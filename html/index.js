...
async function uploadPhoto(file) {
  try {
    ...
    const response = await fetch(`${cognitoConfig.apiGatewayUrl}/photos`, {
      method: "POST",
      headers: {
...
async function loadPhotos() {
  try {
    ...
    // Call API Gateway endpoint to get photos
    const response = await fetch(`${cognitoConfig.apiGatewayUrl}/photos`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${accessToken}`,
    ...
    // Update global photos array with API response
    photos = result.photos.map((photo) => ({
      id: photo.photo_id,
      fileName: photo.file_name,
      size: photo.file_size,
      type: photo.file_type || "image/jpeg",
      uploadedBy: photo.user_email,
